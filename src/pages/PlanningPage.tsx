  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="stat-card"><p className="text-xs text-muted-foreground mb-1">Custos fixos</p><p className="text-xl font-bold text-expense">{fmt(fixedTotal)}</p><p className="text-xs text-muted-foreground mt-0.5">{fixedCosts.length} item{fixedCosts.length !== 1 ? 's' : ''} cadastrado{fixedCosts.length !== 1 ? 's' : ''}</p></div>
        <div className="stat-card"><p className="text-xs text-muted-foreground mb-1">Salário líquido previsto</p><p className="text-xl font-bold text-income">{fmt(netSalary)}</p>{grossSalary > 0 && <p className="text-xs text-muted-foreground mt-0.5">Bruto: {fmt(grossSalary)}</p>}</div>
        <div className="stat-card"><p className="text-xs text-muted-foreground mb-1">Saldo previsto</p><p className={`text-xl font-bold ${netSalary - fixedTotal >= 0 ? 'text-income' : 'text-expense'}`}>{fmt(netSalary - fixedTotal)}</p>{netSalary > 0 && fixedTotal > 0 && <p className="text-xs text-muted-foreground mt-0.5">{((fixedTotal / netSalary) * 100).toFixed(0)}% do líquido comprometido</p>}</div>
      </div>
