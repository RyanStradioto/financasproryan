...
export default function InsightsPage() {
  ...
  const localInsights = useMemo(() => {
    ...
    if (prevTotalExpenses > 0 && totalExpenses > 0) {
      ...
      if (expDiff > 15) {
        insights.push({ type: 'warning', icon: '📈', title: `Gastos subiram ${expDiff.toFixed(0)}%`, description: `Seus gastos passaram de ${formatCurrency(prevTotalExpenses)} para ${formatCurrency(totalExpenses)}.` });
      } else if (expDiff < -10) {
        insights.push({ type: 'achievement', icon: '📉', title: `Gastos reduziram ${Math.abs(expDiff).toFixed(0)}%`, description: `Você reduziu gastos de ${formatCurrency(prevTotalExpenses)} para ${formatCurrency(totalExpenses)}.` });
      }
    }
    ...
    for (const cat of activeCategories) {
      ...
      if (pct > 100) {
        insights.push({ type: 'warning', icon: '🔴', title: `${cat.icon} ${cat.name}: estourou o orçamento`, description: `Você gastou ${formatCurrency(spent)} de ${formatCurrency(budget)}.` });
      } else if (pct > 80) {
        insights.push({ type: 'tip', icon: '🟡', title: `${cat.icon} ${cat.name}: perto do limite`, description: `Já usou ${pct.toFixed(0)}% do orçamento.` });
      }
    }
    ...
    if (ccTotal > 0 && totalIncome > 0) {
      ...
      if (ccPct > 50) {
        insights.push({ type: 'warning', icon: '💳', title: 'Alto uso de cartão de crédito', description: `Fatura de ${formatCurrency(ccTotal)} representa ${ccPct.toFixed(0)}% da sua renda.` });
      }
    }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold flex items-center gap-2">
          ...
          Consultoria IA
          <span className="text-xs text-muted-foreground font-normal">— só gera quando você clicar no botão</span>
        </h2>
