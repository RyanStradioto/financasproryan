...
import { isNeutralTransfer } from '@/lib/investmentMarker';
...
export default function ReportPage() {
  ...
  const incomeInRange = allIncome.filter(i => {
    ...
    if (isNeutralTransfer(i.notes)) return false; // resgates são transferências patrimoniais, não receita real
  ...
  const expensesInRange = allExpenses.filter(e => {
    ...
    return !isCCMirror(e.notes) && !isBillPayment(e.notes) && !isNeutralTransfer(e.notes);
  });
  ...
  const prevTotals = useMemo(() => {
    ...
    const prevIncome = allIncome
      .filter(i => i.status === 'concluido' && inRange(i.date?.substring(0, 7)) && (accountId === '__all__' || i.account_id === accountId) && !isNeutralTransfer(i.notes))
      .reduce((s, i) => s + Number(i.amount), 0);
    const prevExpNonCC = allExpenses
      .filter(e => e.status === 'concluido' && inRange(e.date?.substring(0, 7)) && (accountId === '__all__' || e.account_id === accountId) && !isCCMirror(e.notes) && !isBillPayment(e.notes) && !isNeutralTransfer(e.notes))
