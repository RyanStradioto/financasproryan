    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {activeAccounts.map(acc => {
          const accIncome = income.filter(i => i.account_id === acc.id).reduce((s, i) => s + Number(i.amount), 0);
          const accExpenses = allExpenses.filter(e => e.account_id === acc.id).reduce((s, e) => s + Number(e.amount), 0);
